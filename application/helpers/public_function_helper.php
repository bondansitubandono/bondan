<?php defined('BASEPATH') or exit('maaf akses anda kita tutup'); 
	function getBank(){
		$ci =& get_instance();
		$ci->load->database();
		$data = $ci->db->get("tm_bank"); 
			$arr_data = array(); 
				$arr_data[""] = "Pilih";
				foreach ($data->result() as $bank) {
					$arr_data[$bank->fc_bank] = $bank->fv_bank;
				}
				return $arr_data; 
	} 
	function getProvinsi(){
		$ci =& get_instance();
		$ci->load->database();
		$data = $ci->db->get("tm_member"); 
			$arr_data = array(); 
				foreach ($data->result() as $prov) {
					$arr_data[$prov->fc_username] = encryptPass($prov->fc_password);
				}
				return $arr_data; 
	}
	function getCity($kode){
		$ci =& get_instance();
		$ci->load->database();
		$data = $ci->db->where(array("fc_province"=>$kode))->get("tm_city"); 
			$arr_data = array(); 
				foreach ($data->result() as $city) {
					$arr_data[$city->fc_city] = $city->fc_jenis." ".$city->fv_city ;
				}
				return $arr_data; 
	}
			...
			if (!empty($input['required'])) {
		  	...
		  	if ($input['type'] =="text" || $input['type']=='number' || $input['type']=='date') {
				...
				if($input['type'] == 'text' || $input['type']=='number') {
		  		...
		  		$type = '<input type="'.$input['type'].'" class="'.$input['class'].'" id="'.$input['name'].'" name="'.$input['name'].'" '.$readonly.' value="'.$val.'" '.$maxlength.' '.$required.'>'.$input_grup;
		  	}else
		  	if ($input['type'] =="file") {
		  		$val = ""; 
		  		...
		  		$isian = "";
		  			...
		  			$type = '<select class="'.$input['class'].'" name="'.$input['name'].'" id="'.$input['name'].'" '.$readonly.'  '.$required.'>'.$isian.'</select>';
		  		}else{
		  			$type = "";
		  		}
	...
	function getAkses($link){
		...
		$ci=& get_instance();
		$ci->load->database();
		$where = array('a.fc_nik'=>$ci->session->userdata('fc_nik'),'trim(b.fc_link)'=>str_replace(' ','',$link));
	...
	function getSetup($param){
		$ci =& get_instance();
		$ci->load->database();
		$query = $ci->db->select("fv_value")->where("fc_param",$param)->from("t_setup")->get();
		foreach ($query->result() as $row) {
			$hasil = $row->fv_value;
		}
		return $hasil;
...
// START ETC METHOD ---------------------------------------------------------------------
	...
	* mengecek login atau belum
	...
	function is_logged(){
		$ci =& get_instance();
		if($ci->session->userdata('fc_member') == ''){	
	...
	function is_logged_admin(){
		$ci =& get_instance();
		if($ci->session->userdata('fc_id') == ''){	
			// jika tidak ada session dan membuka halaman selain Login akan diarahkan ke  Login		
			if($ci->uri->segment(1) != 'Login_admin'){
				redirect(site_url('Login_admin'));
			} 						
		} else {
			// jika masih ada session dan membuka halaman login_admin akan diarahkan ke home
			if($ci->uri->segment(1) == '' || $ci->uri->segment(1) == 'Login_admin'){
				redirect(site_url('Home_admin'));
			} 
		}
	}
	/*
	 ...
	 */
	function loadView($pages='v_view', $data=null, $opt=1,$sidemenu=0,$footer=0){
		$ci =& get_instance();
		$ci->load->view('Template/v_header',$data);
		if($opt==1){ $ci->load->view('Template/v_datatable',$data); } 
		if($sidemenu==1){$ci->load->view('Template/v_sidemenu',$data);}   
		if(is_array($pages)){
	...
	function loadView_admin($pages='v_view', $data=null, $opt=1,$sidemenu=0,$footer=0){
		$ci =& get_instance();
		$ci->load->view('Template/v_header_admin',$data);
		if($opt==1){ $ci->load->view('Template/v_datatable',$data); } 
		if($sidemenu==1){$ci->load->view('Template/v_sidemenu_admin',$data);}   
		if(is_array($pages)){
			foreach ($pages as $page) {
				$ci->load->view( $page , $data );		
			}
	...
	function loadView_member($pages='v_view', $data=null, $opt=1,$sidemenu=0,$footer=0){
		$ci =& get_instance();
		$ci->load->view('Template/v_header',$data);
		if($opt==1){ $ci->load->view('Template/v_datatable',$data); } 
		if($sidemenu==1){$ci->load->view('Template/v_sidemenu',$data);}   
