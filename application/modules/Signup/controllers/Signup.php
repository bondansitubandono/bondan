...
//            'img_height' => 30,
	...
	public function Cekkode(){
		...
		$date = date("Y-m-d");
		$sql = "SELECT * FROM t_referral WHERE fd_join= '$date' AND fc_hold='N' ORDER BY fc_referral DESC  ";
		...
		$where2 = array("fc_referral_id" => $tmp);
		$proses2 = $this->M_signup->update($data2,$where2);
	...
	public function Edit($data){ 
		...
		public function Simpan($data){ 
			$act=$this->input->post("ref_code2");
			if (!empty($act)) {
				$ref_code = $this->input->post("ref_code2");
			}
			else{
